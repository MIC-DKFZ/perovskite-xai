...
if __name__ == '__main__':
    ...
    parser.add_argument('--undecay_norm', action='store_true',
    ...
    parser.add_argument('--T_max', default=None, type=int,
    ...
    parser.add_argument('--label_smoothing', default=0.0, type=float,
                        help='Label Smoothing parameter, range:0.0-1.0, the higher the more smoothing, default applies'
                             'no smoothing')
    parser.add_argument('--stochastic_depth', default=0.0, type=float,
                        help='Dropout rate for stochastic depth, only for ResNet-like models, default applies no'
                             'stochastic depth')
    parser.add_argument('--final_layer_dropout', default=0.0, type=float,
    ...
    parser.add_argument('--shakedrop', action='store_true',
    ...
    parser.add_argument('--bottleneck', action='store_true', help='Whether to use bottleneck building '
                                                                  'blocks for ResNet')
    # Seeding
    ...
    parser.add_argument('--data_dir', default=os.environ[
        'DATASET_LOCATION'] if 'DATASET_LOCATION' in os.environ.keys() else '/home/s522r/Desktop/perovskite/new_data/2021_KIT_PerovskiteDeposition/preprocessed',
                        help='Location of the dataset')
    parser.add_argument('--exp_dir', default=os.environ[
        'EXPERIMENT_LOCATION'] if 'EXPERIMENT_LOCATION' in os.environ.keys() else './experiments',
    ...
    parser.add_argument('--chpt_name', default='',
    ...
    selected_data_dir = data_dir if 'DATASET_LOCATION' not in os.environ.keys() else os.path.join(data_dir, 'Perovskite_preprocessed')
    ...
    for fold in range(5):
        ...
        checkpoint_callback = ModelCheckpoint(
                dirpath=chpt_dir,
                filename=chpt_name+'-{epoch}-{test_loss:.2f}-{test_acc:.3f}'
            )
        ...
        trainer = pl.Trainer(logger=None,
                             ...
                             callbacks=[TimerCallback(params['epochs'], args.gpu_count)],
        ...
        validation_metrics.append(
            (run.info.run_id, (train_MSE, val_MSE, train_MAE, val_MAE)))
