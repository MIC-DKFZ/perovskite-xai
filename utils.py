...
def detect_misconfigurations(model_name, args):
    ...
    assert not (args.ASAM and args.SAM), 'Either use adaptive (--ASAM) or non-adaptive (--SAM) Sharpness Aware Minimization'
    if args.nesterov:
    ...
    if args.T_max:
        assert args.scheduler == 'CosineAnneal', 'T_max is a parameter of the Cosine Annealing Scheduler, ' \
                                                 'but you specified {}'.format(args.scheduler)

    if args.mixup_alpha != 0.2:
        assert args.mixup, 'Mixup has to be True for specifying the mixup alpha parameter'
    # Model specific settings
    if args.bottleneck:
        assert 'ResNet' in model_name or 'Pyramid' in model_name and '18' not in model_name and \
               '34' not in model_name, 'Bottleneck not available for {}'.format(model_name)
    if args.zero_init_residual or args.se or args.shakedrop:
        assert 'ResNet' in model_name or 'WRN' in model_name or 'Pyramid' in model_name, 'specified regularization ' \
                                                                                         'only for ResNet-like models'
...
def get_params_to_log(params, model_name):
    ...
    params_to_log = {'batch_size': params['batch_size'],
