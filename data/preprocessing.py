...
def preprocess(rootPath):
    ...
    for split in ["train", "test"]:
        ...
        data = pd.read_hdf(
            os.path.join(rootPath, "{}_allLabels.h5".format(split)), "df"
        )
        ...
        for substrate in np.unique(data["substrateName"]):
            ...
            substrate_df = pd.read_hdf(
                os.path.join(rootPath, "croppedDataset01_{}.h5".format(substrate)), "df"
            )
            for i, patch in substrate_df.iterrows():
                ...
                images = np.stack(
                    [NDimages, LP725images, LP780images, SP775images]
                ).transpose(
                    (1, 0, 2, 3)
                ...
                maxPL_ND, maxPL_LP725, maxPL_LP780, maxPL_SP775 = (
                    patch["startPLIndicesImg"][2],
                    patch["startPLIndicesImg"][0],
                    patch["startPLIndicesImg"][1],
                    patch["startPLIndicesImg"][2],
                )
